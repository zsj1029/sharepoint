...
class Sharepoint {
  ...
  async fileIsExist(path){
    ...
    const get = type => {
      return axios.get(
          `${url}/_api/web/GetFolderByServerRelativeUrl('${site.serverRelativeUrl}${encodeURIComponent(root)}')/${type}`,
          { headers, responseType: 'json' }
      )
    }
  ...
  async downloadFile(path,savePath=null){
    ...
    try{
      const file = await axios.get(`${url}/_api/web/GetFileByServerRelativeUrl('${encodeURIComponent(path)}')/$value`,
